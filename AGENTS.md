...
Private UI presentation uses one standing `ui-editing` permission, not a per-button/property authorization. `application-presentation` discovers static host surfaces at build time. Preserve the generic inspection/edit/observation/undo path and do not replace it with named-button tools or shared source edits. `npm run build` regenerates the UI catalogue. Dynamic client content, actions, inputs and login are not text-editing surfaces; Atlas iframe content uses its separate bridge. Saved configuration is not visual verification. Report browser observations with their revision and viewport. See `docs/afp-private-ui.md`.
