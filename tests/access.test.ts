...
for (const backend of ["local", "s3"])
  test(`${backend}: tenant and client boundaries; uploads retain bytes; approval does not approve placement`, async () => {
    ...
    const app = createApp(db, {
      ...
      evidenceStorage:
        backend === "s3"
          ? createEvidenceStorage(dataDir, s3Config, fakeS3().transport)
          : undefined,
      send: async () => {},
    });
