...
The persistent row key is `(authenticated user, authenticated firm, feature ID)`. A host-built manifest, digest, definition version, record revision, value, state and timestamp are stored with it. Every set validates the current manifest schema, current policy, protected boundaries, ownership and exact application release. Every read revalidates the saved manifest and digest. A disabled/removed/incompatible preference resolves to the default; no other user inherits it, including another attorney in the same firm or a super admin.

Policy changes or an application release change can make an enabled preference stale. Its saved value remains inspectable but does not apply. Ask the Coordinator to set it again to validate explicitly against the new build/policy; no silent reconciliation occurs. Disabling/removing is available to the authenticated owner even when the old permission is now blocked. Both clear the current custom value. Removal retains a tombstone and required audit history. There is no general Activate button or executable runtime.

Settings → AFP Management → AFP Features displays the current super admin's own configuration, owner/scope, manifest/build/version, permission, compatibility, current/effective value, controls and history. Ordinary staff can set or remove their own preference through the Coordinator and scoped endpoints; they do not gain AFP Management access.
